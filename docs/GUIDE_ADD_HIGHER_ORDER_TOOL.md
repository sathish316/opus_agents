...
class ClockwiseHigherOrderTool(HigherOrderTool):
    ...
    def __init__(self, config_manager=None, instructions_manager=None, model_manager=None):
        ...
        #self.clockwise_helper = ClockwiseCalendarHelper()
...
Add higher order tool to an existing or new agent
...
TODO: Debugging to ensure MCP server is added, Higher order tool is added, and tools called for a query
...
You've created a production-ready higher order tool that uses Clockwise MCP server and tools to schedule deep work tasks. This pattern can be adapted for any MCP server/tools that you want to use directly or enhance and use in your AI agent workflows.
