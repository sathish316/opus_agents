...
The system SHALL read its configuration from `~/.opusai/opus-config.yaml`, seeded from a sample config shipped with the project.

#### Scenario: Config loaded at startup
- **WHEN** the agent starts and `~/.opusai/opus-config.yaml` exists
